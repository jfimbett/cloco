...
\paragraph{Derivation sketch.} These identities follow from integration by parts using $\varphi'(z)=-z\varphi(z)$. For \eqref{eq:tail1}: $\int_a^\infty z\varphi(z)dz = -\varphi(z)\big|_a^\infty = \varphi(a)$. For \eqref{eq:tail2}: write $z^2\varphi(z)=-z\cdot\varphi'(z)$ and integrate by parts to get $-z\varphi(z)\big|_a^\infty + \int_a^\infty\varphi(z)dz = a\varphi(a) + [1-\Phi(a)]$. Higher moments follow recursively.

\paragraph{Application to our model.} Setting $a=-S$ and defining $X:=S+Z$, note that $\{X\ge 0\} = \{Z \ge -S\}$. Therefore:
\begin{align}
\Pr[Z\ge -S] &= \Phi(S),\\
...
\begin{align}
A &= A_0(1+r) + F(r-r_f) \nonumber\\
&= A_0(1+r_f+\sigma X) + f_1\sigma X + f_2\sigma^2 X^2 I \nonumber\\
&= A_0(1+r_f) + (A_0+f_1)\sigma X + f_2\sigma^2 X^2 I \nonumber\\
&= A_0(1+r_f) + \Kcal\,\sigma X + f_2\,\sigma^2 X^2 I.
\end{align}
...
C_1 &:= (S^2+1)\,\Phi(S) + S\,\varphi(S).\label{eq:C1}
\end{align}

\paragraph{Variance of AUM.}
...
\begin{align}
\V[A] &= \Kcal^2\,\sigma^2\,\V[X] + 2\,\Kcal f_2\,\sigma^3\,\Cov(X, X^2 I) + f_2^2\,\sigma^4\,\V(X^2 I) \nonumber\\
&= \Kcal^2\,\sigma^2 + 2\,\Kcal f_2\,\sigma^3\,\Cov(X, X^2 I) + f_2^2\,\sigma^4\,\V(X^2 I).\label{eq:VarA}
\end{align}
...
\begin{align}
\Cov(X, X^2 I) &= \E[X \cdot X^2 I] - \E[X]\,\E[X^2 I] \nonumber\\
&= \E[X^3 I] - S\,\E[X^2 I] \nonumber\\
&= (S^3+3S)\Phi(S) + (S^2+2)\varphi(S) - S\,C_1.\label{eq:Cov}
\end{align}
Similarly, for the variance of $X^2 I$:
\begin{align}
\V(X^2 I) &= \E[(X^2 I)^2] - [\E[X^2 I]]^2 \nonumber\\
&= \E[X^4 I] - C_1^2 \nonumber\\
&= (S^4+6S^2+3)\Phi(S) + (S^3+5S)\varphi(S) - C_1^2.\label{eq:VarX2I}
\end{align}

\paragraph{Derivatives in $\sigma$.}
Differentiate with respect to $\sigma$:
\begin{align}
\partial_\sigma \E[A] &= \Kcal\,S + 2 f_2\,\sigma\,C_1,\label{eq:dEA}\\
\partial_\sigma \V[A] &= 2\Kcal^2\sigma + 6\Kcal f_2\,\sigma^2\,\Cov(X, X^2 I) + 4 f_2^2\,\sigma^3\,\V(X^2 I).\label{eq:dVarA}
\end{align}
Define
\begin{equation}
...
\[
\max_\sigma \; U_M(\sigma,\phi) = \phi\,\E[A] - \frac{\eta\phi^2}{2}\,\V[A],
\]
where $\eta>0$ is the manager's risk aversion coefficient. Taking the first-order condition with respect to $\sigma$ and using the derivatives from \eqref{eq:dEA} and \eqref{eq:dVarA}:
\begin{align}
...
&= 0.\label{eq:manager-FOC}
\end{align}
The left side represents the marginal net benefit of increasing volatility: the first term captures the increase in expected fee revenue (through both higher expected returns when $S>0$ and convex flow effects when $f_2>0$), while the second term captures the disutility from increased fee revenue risk.

When $\eta\phi\cdot 2\Kcal^2 > 2f_2 C_1$, the quadratic term $-\eta\phi^2\cdot 2\Kcal^2\sigma$ dominates for large $\sigma$, making \eqref{eq:manager-FOC} a strictly decreasing function of $\sigma$. Since the FOC is positive at $\sigma=0$ (when $S>0$) and eventually negative for large $\sigma$, the intermediate value theorem guarantees a unique interior solution $\sigma^*(\phi)>0$.

\paragraph{Closed-form approximation.}
Under the linearized variance slope $\partial_\sigma \V[A]\approx 2\Kcal^2\sigma$, the closed-form best response is
\begin{equation}
\label{eq:sigma-star}
\sigma^*(\phi) = \frac{\Kcal S}{2\,(\eta\phi\,\Kcal^2 - f_2 C_1)},\qquad \text{valid when } \eta\phi\,\Kcal^2> f_2 C_1.
\end{equation}

\paragraph{IC fee implementing $\sigma_d$.}
Incentive compatibility requires the manager’s FOC to hold at $\sigma=\sigma_d$, yielding
\begin{equation}
\label{eq:phi-IC}
\phi(\sigma_d)
\;=\;
...
\]
the investor chooses the target volatility $\sigma_d$ to maximize $U_I(\sigma_d,\phi(\sigma_d))$, where the fee $\phi(\sigma_d)$ is pinned down by the incentive compatibility constraint \eqref{eq:phi-IC}.
...
&= \Big[(1-\phi)S - \gamma(1-\phi)^2\sigma_d\Big] + \Big[-(r_f+S\sigma_d) -1 + \gamma(1-\phi)\sigma_d^2\Big]\frac{d\phi}{d\sigma_d}.\label{eq:investor-FOC}
\end{align}
Setting this equal to zero gives the equilibrium condition. The first bracketed term is the \textbf{direct effect}: increasing volatility raises expected net returns (by $S$) but also increases risk. The second term is the \textbf{contract adjustment}: changing the target requires adjusting the fee via the IC constraint, which affects investor utility through both the mean (by reducing the share of returns) and the variance (by reducing exposure to fund risk).
...
\end{align}
where $N(\sigma)$ is the numerator and $D(\sigma)$ is the denominator of the IC fee \eqref{eq:phi-IC}.

Plugging $\phi(\sigma)$ and $\phi'(\sigma)$ into \eqref{eq:investor-FOC} and defining
\begin{equation}
...
\paragraph{Linear rule for the IC fee (details).}
...
\end{equation}
The parameter $\alpha$ acts as a "base fee" and $\beta$ represents the marginal fee per unit of target volatility. This affine structure makes the contract transparent and easy to implement: the investor and manager agree on a volatility target $\sigma_d$, and the fee is computed as $\phi = \alpha + \beta\sigma_d$. This is the basis for Eq.~(\ref{eq:phi-linear-approx}) in the main text. As shown in Figure~\ref{fig:affine-approx}, this approximation is highly accurate within $\pm 5\%$ of the optimal target, with average absolute errors around 0.1\%.
...
\]
In our model, fees are charged on end-of-period AUM at rate $\phi$, so the investor receives $(1-\phi)A$ from terminal AUM $A=A_0(1+r)$, giving a net return of
\[
...
To find the optimal target $\sigma_d^*$, solve $G(\sigma)=0$ where $G$ is defined in Section~\ref{app:investor}. We recommend bisection on an interval $[\underline\sigma,\overline\sigma]$ where:
\begin{itemize}
\item $\underline\sigma > 0$ is chosen small (e.g., $0.01$ or 1\% annualized) to avoid numerical issues near zero.
\item $\overline\sigma$ is chosen as an upper bound where the investor's direct utility gain from volatility becomes negative. A conservative choice is to set $\overline\sigma$ such that $(1-\phi)S - \gamma(1-\phi)^2\overline\sigma < 0$ when ignoring the contract adjustment term. For instance, with $\phi\approx 0.02$, $S=0.5$, $\gamma=3$, this gives $\overline\sigma \approx 0.5/(3\cdot 0.98) \approx 0.17$. In practice, we use $\overline\sigma = 0.40$ to be conservative.
\end{itemize}
Bisection is robust and converges rapidly since $G$ is continuous and typically has a unique zero in the relevant range.

\paragraph{Precomputing tail blocks.}
...
The baseline parameter values for the numerical exercise are $S=0.5$, $r_f=0.02$, $\gamma=3$, $\eta=2$, $A_0=1$, $f_1=0.2$, $f_2=0.5$. Comparative statics sweep the following ranges:
\begin{itemize}
\item Convexity $f_2 \in [0.02, 0.25]$ 
\item Initial AUM $A_0 \in [0.5, 3.0]$
\item Linear flow $f_1 \in [0.05, 0.45]$
\item Sharpe ratio $S \in [0.1, 0.9]$
\item Investor risk aversion $\gamma \in [1, 8]$
\item Manager risk aversion $\eta \in [0.5, 6]$
\end{itemize}
These ranges cover empirically plausible scenarios while ensuring the FOCs remain well-behaved.

\paragraph{Affine approximation.}
To construct the linear approximation $\phi(\sigma_d) \approx \alpha + \beta\sigma_d$, first solve for the optimal target $\sigma_d^*$, then compute $\phi(\sigma_d^*)$ and $\phi'(\sigma_d^*)$ using the formulas in Sections~\ref{app:manager-solution} and \ref{app:investor}. The approximation is then $\alpha = \phi(\sigma_d^*) - \beta\sigma_d^*$ and $\beta = \phi'(\sigma_d^*)$. As Figure~\ref{fig:affine-approx} demonstrates, this approximation remains accurate within $\pm 5\%$ of $\sigma_d^*$ with average errors around 0.1\%.
