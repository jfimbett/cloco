...
\[
\max_\sigma \; U_M(\sigma,\phi) = \phi\,\E[A] - \frac{\eta}{2}\phi^2\,\V[A],
\]
...
\begin{align}
\frac{\partial U_M}{\partial \sigma} &= \phi\,\partial_\sigma \E[A] - \frac{\eta}{2}\phi^2\,\partial_\sigma \V[A] \nonumber\\
&= \phi\Big(\Kcal S + 2 f_2\,\sigma\,C_1\Big) - \frac{\eta}{2}\phi^2\Big(2\Kcal^2\sigma + \Delta_{\text{quad}}\Big) \nonumber\\
...
Under the condition $\eta\phi\,\Kcal^2 > 2f_2 C_1$, the marginal cost grows faster than the marginal benefit for large values of $\sigma$. As a result, the term $-\frac{\eta}{2}\phi^2\cdot 2\Kcal^2\sigma = -\eta\phi^2\Kcal^2\sigma$ dominates for large $\sigma$, making \eqref{eq:manager-FOC-app} strictly decreasing. Since the FOC is positive at $\sigma=0$ (when $S>0$) and eventually negative for large $\sigma$, the intermediate value theorem guarantees a unique interior solution $\sigma^*(\phi)>0$.

\textit{Sufficiency condition.} To verify that the IC fee \eqref{eq:phi-IC-app} satisfies the above condition, substitute $\phi = \phi(\sigma_d)$ from \eqref{eq:phi-IC-app} into $\eta\phi\,\Kcal^2 > 2f_2 C_1$. In practice, this condition is verified numerically across the calibrated parameter ranges in Appendix~\ref{app:numerical}; it holds whenever $\Kcal\,S > 0$, which is ensured by $S>0$ and $A_0, f_1 > 0$.

\textit{Second-order condition.} The manager's objective $U_M(\sigma,\phi) = \phi\E[A] - \frac{\eta}{2}\phi^2\V[A]$ has second derivative
\begin{equation}
\frac{\partial^2 U_M}{\partial\sigma^2} = \phi\,\partial_\sigma^2\E[A] - \frac{\eta}{2}\phi^2\,\partial_\sigma^2\V[A], \label{eq:SOC-UM}
\end{equation}
where, from the formulas in Appendix~\ref{app:variance}:
\[
\partial_\sigma^2\E[A] = 2f_2 C_1 > 0,
\]
\[
\partial_\sigma^2\V[A] = 2\Kcal^2 + 12\Kcal f_2\sigma\,\Cov(X,X^2 I) + 12 f_2^2\sigma^2\,\V(X^2 I) > 0.
\]
The SOC $\partial^2_\sigma U_M < 0$ requires
\begin{equation}
\frac{\eta}{2}\phi\Bigl(2\Kcal^2 + 12\Kcal f_2\sigma\,\Cov(X,X^2 I) + 12 f_2^2\sigma^2\,\V(X^2 I)\Bigr) > 2f_2 C_1. \label{eq:SOC-condition}
\end{equation}
This condition is \emph{independent} of the IC condition \eqref{eq:manager-FOC-app}: the IC condition pins down $\phi(\sigma_d)$ via the first-order condition at $\sigma_d$, whereas \eqref{eq:SOC-condition} is an additional second-order requirement at that point. Numerically, \eqref{eq:SOC-condition} holds strictly throughout the calibrated parameter grid in Appendix~\ref{app:numerical}.

\textit{Global incentive compatibility.} Local optimality (SOC) does not rule out multiple stationary points in $\sigma$. To confirm that $\sigma_d$ is the \emph{global} maximizer of $U_M(\sigma;\phi(\sigma_d))$ over $\sigma\in[\underline\sigma,\overline\sigma]$, we evaluate $U_M(\sigma;\phi(\sigma_d))$ over a dense grid of $\sigma$ values for each parameter combination in the sweep of Appendix~\ref{app:numerical}. The manager's utility profile is everywhere single-peaked with a unique maximum at $\sigma_d$. Figure~\ref{fig:global-ic} shows the baseline-parameter utility profile confirming the unique peak; the second panel verifies the SOC is negative across the full $f_2$ sweep.
...
\begin{equation}
\label{eq:sigma-star}
\sigma^*(\phi) = \frac{\Kcal S}{\eta\phi\,\Kcal^2 - 2f_2 C_1},\qquad \text{valid when } \eta\phi\,\Kcal^2> 2f_2 C_1.
\end{equation}
...
\phi(\sigma_d)
\;=\;
\frac{2\,\partial_\sigma \E[A]\big|_{\sigma_d}}
{\eta\,\partial_\sigma \V[A]\big|_{\sigma_d}}
\;=\;
\frac{2\Big(\Kcal\,S + 2 f_2 \,\sigma_d\,C_1\Big)}
...
&= \Big[(1-\phi)S - \gamma(1-\phi)^2\sigma_d\Big] + \Big[-(1+r_f+S\sigma_d) + \gamma(1-\phi)\sigma_d^2\Big]\frac{d\phi}{d\sigma_d}.\label{eq:investor-FOC-app}
\end{align}
...
\begin{equation}
G(\sigma) := (1-\phi(\sigma))S - \gamma(1-\phi(\sigma))^2\sigma + \Big[-(1+r_f+S\sigma) + \gamma(1-\phi(\sigma))\sigma^2\Big]\phi'(\sigma),
\end{equation}
...
\item $\overline\sigma$ is chosen as an upper bound where the investor's direct utility gain from volatility becomes negative, i.e., $(1-\phi)S - \gamma(1-\phi)^2\overline\sigma < 0$ when ignoring the contract adjustment term. In practice, we use $\overline\sigma = 0.40$, which conservatively brackets the equilibrium target under all parameter ranges considered in Appendix~\ref{app:numerical}.
\end{itemize}
...
The baseline parameter values for the numerical exercise are $S=0.35$, $r_f=0.037$, $\gamma=5$, $\eta=15$, $A_0=1$, $f_1=1.5$, $f_2=25$. Comparative statics sweep the following ranges:
\begin{itemize}
...
\item Manager risk aversion $\eta \in [3, 30]$
\end{itemize}
